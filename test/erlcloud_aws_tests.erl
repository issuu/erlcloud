...
start() ->
    ...
    % meck:expect(erlcloud_httpc, request, fun(_) -> {ok, {{0, 200, 0}, 0, ok}} end),
    meck:expect(erlcloud_httpc, request, fun(_, _, _, _) -> {ok, {{0, 200, 0}, 0, ok}} end),
...
get_url_from_history([{_, {erlcloud_httpc, request, [_, {Url, _}, _, _]}, _}, _CallToExtractBody]) ->
    Url.
